data<-read.table('a2b2.fl.votu_vvotu_tvotu.num',header = T)
library(reshape2)
library(ggsignif)
library(ggplot2)
data$group <- factor(data$group,levels=c("votu-A2-F","votu-A2-L","votu-B2-F","votu-B2-L",
                                         "vvotu-A2-F","vvotu-A2-L","vvotu-B2-F","vvotu-B2-L",
                                         "tvotu-A2-F","tvotu-A2-L","tvotu-B2-F","tvotu-B2-L"))
ggplot(data)+
  ...
  theme(
    ...
    axis.text.x=element_text(size=rel(1),angle = 45,hjust = 1,face="bold"),
